...
void DetectionResultColumn(const BoundingBox_t * boundingBox, RowIndicator_t rowIndicator)
{
	if (boundingBox->_maxY < boundingBox->_minY) 
	{
		printf("Invalid bounding box");
	}

	//_codewords.resize(boundingBox->_maxY - boundingBox->_minY + 1);
}
...
// finding row numbers for other columns easier
// use row height count to make detection of invalid row numbers more reliable
void DetectionResultColumn::adjustIncompleteIndicatorColumnRowNumbers(const BarcodeMetadata& barcodeMetadata)
{
#ifdef HK_DEBUG
...
bool DetectionResultColumn::getRowHeights(std::vector<int>& result)
{
#ifdef HK_DEBUG
...
// This is example of bad design, a getter should not modify object's state
bool DetectionResultColumn::getBarcodeMetadata(BarcodeMetadata& result)
{
#ifdef HK_DEBUG
