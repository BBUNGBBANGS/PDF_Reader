...
{
#ifdef HK_DEBUG
	std::cout << "CodeWord ReadCodeWord cur = " << &cur << " \n";
#endif
...
SymbolInfo ReadSymbolInfo(BitMatrixCursor<POINT> topCur, POINT rowSkip, int colWidth, int width, int height)
{
	...
	for (auto startCur = topCur; clusterMask != 0b111 && maxAbsComponent(topCur.p - startCur.p) < height / 2; startCur.p += rowSkip) {
		...
		case 3: 
			rows1 = cw.code % 3, res.ecLevel = (cw.code % 30) / 3; 
			break;

		case 6: 
			res.nCols = (cw.code % 30) + 1; 
			break;
...
Reader::decode(const BinaryBitmap& image) const
{
	if (_isPure) 
	{
		auto res = DecodePure(image, _characterSet);
	...
	DecodeStatus status = DoDecode(image, false, results, _characterSet);
